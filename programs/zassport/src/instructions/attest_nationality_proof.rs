...
#[derive(Accounts)]
pub struct AttestNationality<'info> {
    #[account(
        mut,
...
pub fn attest_nationality(
    ...
    // Mark attested
    identity.nationality_verified = true;
    identity.last_attestation_ts = now;

    Ok(())
}

fn build_nat_message(
    program_id: &Pubkey,
    owner: &Pubkey,
    identity: &Pubkey,
    commitment: &[u8; 32],
    ...
    m.extend_from_slice(b"ZASSPORT|NAT|v1");
    ...
    m.extend_from_slice(&allowed_nationality.to_le_bytes());
